...
void store_cpu_topology(unsigned int cpuid)
{
	...
	/* For adjustment to topology later; see RHELSA topology adjustment below */
	...
	if (cpuid_topo->cluster_id > num_clusters)
		num_clusters = cpuid_topo->cluster_id;
 ...
 *  Note: vcpu topology will be correct since only MPIDR.Aff0 used,
 *        and not supporting > 256 vcpu.
 */
static int __init acpi_adjust_cpu_topology(void)
{
	...
	if (acpi_disabled) {
		pr_warn("ACPI disabled; Unsupported sysfs topology generated\n");
	} else { /* ACPI-based topology */
		unsigned int cpu;
		for_each_possible_cpu(cpu) {
			struct cpu_topology *cpu_topo = &cpu_topology[cpu];
			cpu_topo->core_id = cpu_topo->thread_id * num_threads +
						cpu_topo->core_id * num_cores +
						cpu_topo->cluster_id * num_clusters;
			cpu_topo->cluster_id = 0; /* assuming single-socket atm */
